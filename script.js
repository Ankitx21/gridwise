...
const defaultAppliances = [
  ...
  { id: "geyser", name: "Geyser", kind: "comfort", userStart: 7.0, recommendedStart: 6.35, duration: 28, power: 2.0, enabled: true, linked: true, note: "Morning comfort load" },
  { id: "geyser-evening", name: "Geyser evening", kind: "comfort", userStart: 16.0, recommendedStart: 16.0, duration: 30, power: 2.0, enabled: true, linked: true, note: "Evening hot water slot" },
...
function timelineSegments(item) {
  ...
  const start = ((item.recommendedStart % 24) + 24) % 24;
  const finish = start + item.duration / 60;
  const label = `${formatHour(item.recommendedStart)} - ${formatHour(endTime(item))}`;
