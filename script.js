...
const defaultAppliances = [
  { id: "washing", name: "Washing machine", kind: "flexible", userStart: 18.25, recommendedStart: 12.75, duration: 70, power: 0.7, enabled: true, linked: true, note: "Laundry can finish anytime today" },
  { id: "dishwasher", name: "Dishwasher", kind: "flexible", userStart: 20.0, recommendedStart: 14.1, duration: 95, power: 1.15, enabled: true, linked: true, note: "Shifted out of 18:00-22:00 peak" },
  { id: "geyser", name: "Geyser", kind: "comfort", userStart: 7.0, recommendedStart: 6.35, duration: 28, power: 2.0, enabled: true, linked: true, note: "Morning and evening hot water slots", userSlots: [{ start: 7.0, duration: 28 }, { start: 16.0, duration: 30 }], slots: [{ start: 6.35, duration: 28 }, { start: 16.0, duration: 30 }] },
  { id: "bedroom-ac", name: "Bedroom AC", kind: "comfort", userStart: 15.0, recommendedStart: 15.0, duration: 60, power: 1.35, enabled: true, linked: true, note: "Afternoon cooling load" },
  { id: "water-pump", name: "Water pump", kind: "flexible", userStart: 19.5, recommendedStart: 11.8, duration: 25, power: 0.75, enabled: true, linked: true, note: "Tank top-up during low-cost window" },
  { id: "ev-charger", name: "EV charger", kind: "flexible", userStart: 23.5, recommendedStart: 23.5, duration: 150, power: 2.2, enabled: true, linked: true, note: "Late-night charging slot" },
  { id: "refrigerator", name: "Refrigerator", kind: "critical", userStart: 0.0, recommendedStart: 0.0, duration: 1440, power: 0.12, enabled: true, linked: true, note: "Always-on base load" }
];
...
const elements = {
  ...
  deviceAppliance: document.querySelector("#deviceAppliance"),
...
function scheduledSlots(item, startField = "recommendedStart") {
  const slotKey = startField === "userStart" ? "userSlots" : "slots";
  if (Array.isArray(item[slotKey]) && item[slotKey].length) return item[slotKey];
  return [{ start: item[startField], duration: item.duration }];
}

function slotEnd(slot) {
  return slot.start + slot.duration / 60;
}

function formatSlot(slot) {
  return `${formatHour(slot.start)} - ${formatHour(slotEnd(slot))}`;
}
...
function energy(item) {
  return item.power * scheduledSlots(item).reduce((sum, slot) => sum + slot.duration / 60, 0);
}

function runCost(item, startField) {
  return scheduledSlots(item, startField).reduce((slotTotal, slot) => {
    let remaining = slot.duration / 60;
...
function defaultDeviceType(item) {
  const name = item.name.toLowerCase();
  if (name.includes("ac")) return "AC controller";
  if (name.includes("ev charger") || name.includes("charger")) return "EV charger API";
  if (item.kind === "critical") return "Smart plug";
  return "Smart switch";
}

function defaultDeviceStatus(item) {
  if (!item.enabled) return "Offline";
  if (item.kind === "critical") return "Online";
  return "Standby";
}

function deviceMeta(item) {
  return {
    type: item.deviceType || defaultDeviceType(item),
    status: item.deviceStatus || defaultDeviceStatus(item),
    lastSeen: item.lastSeen || (item.enabled ? "Just now" : "1 hr ago")
  };
}

function registerDevice(item, type, status) {
  item.linked = true;
  item.deviceType = type || item.deviceType || defaultDeviceType(item);
  item.deviceStatus = status || item.deviceStatus || defaultDeviceStatus(item);
  item.lastSeen = item.deviceStatus === "Offline" ? "1 hr ago" : "Just now";
  item.note = "Registered smart device";
}

function delinkDevice(item) {
  item.linked = false;
  item.deviceStatus = "Offline";
  item.lastSeen = "Not connected";
  item.note = "Manual appliance, not linked to a device";
}
...
function projectedLoadAt(time, candidate = null) {
  return 1.35 + appliances.reduce((sum, item) => {
    const slots = candidate && candidate.id === item.id
      ? [{ start: candidate.start, duration: item.duration }]
      : scheduledSlots(item);
    const isActive = item.kind === "critical" || slots.some((slot) => time >= slot.start && time < slotEnd(slot));
    return item.enabled && isActive ? sum + item.power : sum;
  }, 0);
}
...
function timelineSegments(item) {
  if (item.kind === "critical") {
    return [{ start: 0, end: 24, label: `${formatHour(item.recommendedStart)} - ${formatHour(endTime(item))}` }];
  }

  return scheduledSlots(item).flatMap((slot) => {
    const start = ((slot.start % 24) + 24) % 24;
    const finish = start + slot.duration / 60;
    const label = formatSlot(slot);

    if (finish <= 24) {
      return [{ start, end: finish, label }];
    }

    return [
      { start, end: 24, label },
      { start: 0, end: finish - 24, label: "" }
    ];
  });
}
...
function renderTimeline() {
  ...
  elements.timeline.innerHTML = `
    <div class="time-axis">
      <span></span>
      <div class="timeline-scale">${ticks.map((tick) => `<span class="time-tick" style="left:${(tick / 24) * 100}%">${String(tick).padStart(2, "0")}:00</span>`).join("")}</div>
    </div>
  `;
  ...
  activeAppliances().forEach((item) => {
    ...
    const blocks = timelineSegments(item).map((segment) => {
      const left = (segment.start / 24) * 100;
      const width = ((segment.end - segment.start) / 24) * 100;
      const edgeClass = left > 88 ? " edge-end" : left < 4 ? " edge-start" : "";
      const blockClass = item.kind === "critical" ? item.kind : `${item.kind}${segment.label ? " has-label" : " continuation"}`;
      return `
        <div class="timeline-block ${blockClass}" style="left:${left}%;width:${width}%" aria-label="${item.name}: ${segment.label || "continues"}"></div>
        ${segment.label ? `<span class="timeline-time-label ${item.kind}${edgeClass}" style="left:${left}%">${segment.label}</span>` : ""}
      `;
    }).join("");
  ...
  appliances.forEach((item) => {
    ...
    card.innerHTML = `
      <div class="appliance-main">
        <div class="appliance-title"><strong>${item.name}</strong><span class="pill">${item.kind}</span><span class="pill device-status ${item.linked ? "linked" : "unlinked"}">${item.linked ? "Linked" : "Manual"}</span></div>
...
function renderDevices() {
  const linked = appliances.filter((item) => item.linked);
  elements.registeredCount.textContent = `${linked.length} linked`;
  elements.deviceAppliance.innerHTML = appliances.map((item) => `<option value="${item.id}">${item.name}${item.linked ? " (linked)" : ""}</option>`).join("");
  elements.deviceList.innerHTML = linked.length ? linked.map((item) => {
    const meta = deviceMeta(item);
    return `
      <article class="device-card">
        <div>
          <div class="device-title"><strong>${item.name}</strong><span class="pill device-status linked">${meta.status}</span></div>
          <div class="device-meta">${meta.type} • ${item.power.toFixed(2)} kW peak • Last seen ${meta.lastSeen}</div>
        </div>
        <div class="device-actions">
          <span class="device-reading">${energy(item).toFixed(2)} kWh today</span>
          <button class="small-button link-button is-linked" type="button" data-delink-device="${item.id}">Delink</button>
        </div>
      </article>
    `;
  }).join("") : `<div class="empty-state">No registered devices yet.</div>`;
}
...
function renderSummary() {
  elements.summaryRows.innerHTML = activeAppliances().map((item) => `
    <tr>
      ...
      <td>${scheduledSlots(item).map(formatSlot).join(" / ")}</td>
...
function updateMetrics() {
  ...
  for (let time = 0; time < 24; time += 0.25) {
    const load = active.reduce((sum, item) => {
      ...
      return scheduledSlots(item).some((slot) => time >= slot.start && time < slotEnd(slot)) ? sum + item.power : sum;
    }, 0);
...
elements.deviceList.addEventListener("click", (event) => {
  const id = event.target.dataset.delinkDevice;
  if (!id) return;
...
elements.deviceForm.addEventListener("submit", (event) => {
  ...
  const item = appliances.find((appliance) => appliance.id === elements.deviceAppliance.value);
  if (!item) return;
  registerDevice(item, elements.deviceType.value, elements.deviceStatus.value);
  renderAll();
});
elements.form.addEventListener("submit", (event) => {
  ...
  const item = {
    ...
    enabled: true,
    linked: false,
    note: "User-added manual appliance"
  };
  item.recommendedStart = bestStartFor(item);
